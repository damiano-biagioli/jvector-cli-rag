...
- **Ollama** — LLM locale per la generazione delle risposte
...
| Ollama | qualsiasi recente | con almeno un modello, es. `ollama pull llama3.2` |
...
|---|---|---|
| `--ollama-model <nome>` | `llama3.2` | modello Ollama da interrogare (`ollama list` per l'elenco) |
| `--ollama-url <url>` | `http://localhost:11434` | URL del server Ollama |
...
file .md/.txt ──▶ chunking (~1000 char, overlap 200) ──▶ embeddings ONNX (MiniLM, 384 dim)
                                                              │
                                                              ▼
risposta ◀── Ollama (LLM) ◀── prompt con contesto ◀── top-K chunk simili ◀── indice JVector
                                                              ▲
...
4. **Retrieval + generazione** (`ChatLoop`, `OllamaClient`): i chunk più simili alla domanda
...
├── OllamaClient.java      # client HTTP per l'API chat di Ollama
...
| Problema | Soluzione |
|---|---|
| `Ollama non raggiungibile` | avvia il server con `ollama serve` |
